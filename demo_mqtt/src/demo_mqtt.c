...
static uint32_t MQTT_Gethostbyname(void)
{
    ...
    IP = (const ip_addr_t *)hostentP->h_addr_list[0];
    return IP->addr;
}
...
static int32_t MQTT_Connect(int32_t Socketfd)
{
	...
	TxLen = MQTT_ConnectMsg(&TxBuffer, &PayloadBuffer,
			...
			MQTT_HEAT_TO * 2, DemoClientID, DemoWillTopic, DemoUser, DemoPasswd, DemoWillMsg, strlen(DemoWillMsg));
...
static int32_t MQTT_Subscribe(int32_t Socketfd)
{
	...
	PayloadBuffer.Pos = 0;
	gPackID++;
	TxLen = MQTT_SubscribeMsg(&TxBuffer, &PayloadBuffer, gPackID, DemoSub, sizeof(DemoSub)/sizeof(MQTT_SubscribeStruct));
	if (MQTT_TCPTx(Socketfd, TxLen, MQTT_TCP_TO) < 0)
	{
		return -1;
	}
...
static int32_t MQTT_Unsubscribe(int32_t Socketfd)
{
	...
	PayloadBuffer.Pos = 0;
	gPackID++;
	TxLen = MQTT_UnSubscribeMsg(&TxBuffer, &PayloadBuffer, gPackID, DemoSub, sizeof(DemoSub)/sizeof(MQTT_SubscribeStruct));
	if (MQTT_TCPTx(Socketfd, TxLen, MQTT_TCP_TO) < 0)
	{
		return -1;
	}
